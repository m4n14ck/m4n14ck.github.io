...
function renderNav() {
  ...
  $("missions").innerHTML = missions.map((mission, index) => {
    ...
    return `${moduleHeader}<button class="mission ${index === state.active ? "active" : ""} ${done ? "done" : ""} ${!unlocked(index) ? "locked" : ""}" data-id="${index}">
      <b>${done ? '<span class="ui-icon" data-icon="check" aria-label="Completada"></span>' : pad(index + 1)}</b>
...
$("hint").onclick = () => {
  const box = $("hintBox");
  box.innerHTML = `<b><span class="ui-icon" data-icon="idea" aria-hidden="true"></span> PISTA DEL SISTEMA</b><p>${missions[state.active].hint}</p>`;
...
function finishPractice(reason) {
  ...
  $("startPractice").innerHTML = `INICIAR DE NUEVO · ${formatPracticeTime(practiceLevels[selectedPracticeLevel].seconds)} <span class="ui-icon" data-icon="arrow-right" aria-hidden="true"></span>`;
...
function selectPracticeLevel(levelName) {
  ...
  updatePracticeTimer();
  $("practiceDifficulty").textContent = level.label;
  $("startPractice").innerHTML = `INICIAR COMPETENCIA · ${formatPracticeTime(level.seconds)} <span class="ui-icon" data-icon="arrow-right" aria-hidden="true"></span>`;
...
$("submitPractice").onclick = () => {
  ...
  practiceScore++;
  $("practiceScore").textContent = practiceScore;
  $("practiceFeedback").innerHTML = '<span class="ui-icon" data-icon="check" aria-hidden="true"></span> Solución correcta';
