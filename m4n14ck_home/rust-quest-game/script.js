...
const explanationTokens = {
  ...
  "for": "keyword", "while": "keyword", "fn": "keyword", "push": "keyword",
...
try {
  ...
  if (saved && Array.isArray(saved.done) && Array.isArray(saved.codes)) state = { ...state, ...saved };
} catch {}
...
function renderSyntax() {
  ...
  const tokenPattern = /\/\/.*$|"(?:\\.|[^"\\])*"|\b(?:println!|print!|vec!)|\b(?:fn|let|mut|const|if|else|match|for|in|while|struct|enum|return|pub|use|impl|self|loop|break|continue|as)\b|\b(?:String|Result|Vec|Option|Some|None|Ok|Err|i32|u32|u16|f64|bool|str)\b|\b\d+(?:\.\d+)?\b/gm;
  let html = "";
  let lastIndex = 0;
...
$("next").onclick = () => {
  if (state.active < missions.length - 1) {
    ...
    window.scrollTo({ top: document.querySelector(".game").offsetTop - 90, behavior: "smooth" });
  }
};
